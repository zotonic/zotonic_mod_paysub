...
subscription_terms([ {<<"status">>, Status} | Ts ], Ws, Args, Context) ->
    S = case Status of
        <<"uncanceled">> -> m_paysub:all_states() -- [ <<"canceled">> ];
        <<"allaccess">> -> m_paysub:access_states(Context);
        <<"allactive">> -> m_paysub:active_states();
        <<"allpending">> -> m_paysub:pending_states(Context);
        _ -> [ Status ]
    end,
    Args1 = [ S | Args ],
